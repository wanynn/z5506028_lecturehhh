...
ROOTDIR = os.path.join('/Users/wanynn/PycharmProjects/Toolkit', 'project1')
...
def get_tics(pth):
    ...
    with open(pth,'r') as file:
        for line in file:
            if line.strip():
                exchange,ticket= line.strip().split('=')
                exchange= exchange.strip('"')
                ticket= ticket.strip('"')
                exchange= exchange.lower()
                ticket=ticket.lower()
                tic_exchange_dic[ticket] = exchange
...
def read_dat(tic):
    ...
    filepath=os.path.join(DATDIR,f'{tic}_prc.dat')
    tic_infor=[ ]
    with open(filepath,'r') as file:
        tic_infor=file.read().split('\n')
...
def line_to_dict(line):
    ...
    for column in COLUMNS:
        ...
        value=line[i:i+width].strip()
        infor_dict[column]=value
        i=i+width
    return infor_dict
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        if not tickers_lst:
            raise Exception(" The ticket_lst is an empty list.")
        else:
            for ticket in tickers_lst:
                 if ticket not in tic_exchange_dic:
                     raise Exception(f"The ticket '{ticket}' is not a key of tic_exchange_dic.")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        if not col_lst:
            raise Exception("The col_lst is an empty list.")
        else:
            for columns_ in col_lst:
                 if columns_ not in COLUMNS:
                     raise Exception(f"'{columns_}' is not found in 'COLUMNS'.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for ticker in tickers_lst:
        ...
        # Read the data for the ticker using read_dat
        lines = read_dat(ticker)
        for line in lines:
            line_dict = line_to_dict(line)
            if col_lst is not None:
                # Filter the line_dict to contain only the specified columns in col_lst
                line_dict = {col: line_dict[col] for col in col_lst}

            ticker_data.append(line_dict)
